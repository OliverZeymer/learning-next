...
const ExploreCard = ({ id, imgUrl, title, index, activeCard, handleClick }) => {
  ...
  return (
    ...
    relative ${
    ...
    } flex items-center justify-center text-white min-w-[170px] h-[700px] transition-[flex] duration-[0.7s] ease-out-flex cursor-pointer`}
      ...
      />
      {activeCard !== id ? (
        <h3 className="font-semibold sm:text-2xl text-lg lg:bottom-20 absolute z-0 lg:rotate-[-90deg] lg:origin-[0,0]">
          {title}
        </h3>
      ) : (
        <motion.div className="absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-3xl">
          ...
          <p className="font-normal text-lg leading-5 text-white uppercase">
            Enter the Metaverse
          </p>
